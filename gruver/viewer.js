...
function handleMapMenuCollapse(e) {
    const toggle = e.target.closest('.leaflet-control-layers-toggle');
    const container = e.target.closest('.leaflet-control-layers');

    if (toggle && container && container.classList.contains('leaflet-control-layers-expanded')) {
        // Force collapse on mobile when clicking the "X" toggle while expanded
        if (window.innerWidth < 768) {
            e.preventDefault();
            e.stopPropagation();
            e.stopImmediatePropagation(); // Prevent Leaflet or Map from seeing this

            if (layerControl) {
                try { layerControl.collapse(); } catch (err) { console.log(err); }
            }
            container.classList.remove('leaflet-control-layers-expanded');
            console.log("Map menu forced closed.");
        }
    }
}

document.addEventListener('click', handleMapMenuCollapse, true);
document.addEventListener('touchstart', handleMapMenuCollapse, { passive: false, capture: true });
...
// --- ACCESSIBILITY PATCH FOR LEAFLET GROUPED LAYERS ---
// The plugin uses <label> for group titles without associated inputs, causing console errors.
// We patch _addItem to use <span> for group labels instead.
if (L.Control.GroupedLayers) {
    ...
    L.Control.GroupedLayers.prototype._addItem = function (obj) {
        ...
        if (obj.group) {
            // Find the group label and swap tag from <label> to <span>
...
function initMap() {
    ...
    // Auto-zoom when adding layers with minZoom requirement
    ...
    const nibOptions = (layer) => ({
        ...
        version: '1.1.1', // KRITISK: Må være 1.1.1 for korrekt Web Mercator støtte
        maxZoom: 18,
        tileSize: 256,
        updateWhenIdle: true,
        detectRetina: false
    ...
    const flyfotoLayers = {
        "Flyfoto (Ortofoto)": L.tileLayer('nib_proxy.php?svc=nib&service=WMTS&request=GetTile&version=1.0.0&layer=Nibcache_web_mercator_v2&style=default&tilematrixset=GoogleMapsCompatible&tilematrix={z}&tilerow={y}&tilecol={x}&format=image/jpeg', {
            attribution: '© Norge i bilder',
            maxZoom: 22,        // Tillat digital zoom helt inn
            maxNativeZoom: 18,  // Høyeste faktiske oppløsning fra server
            tileSize: 256
        }),
        "--- Kommer senere ---": L.layerGroup(), // Separator
        "Flyfoto (Alternativ)": L.tileLayer.wms('nib_proxy.php?svc=nib', nibOptions('ortofoto')), // Fallback to proxy
    ...
    const groupedOverlays = {
        ...
        "Historiske Kart (Høy Oppløsning)": {
            // --- VERIFISERT OG STABIL HISTORISK TJENESTE ---
            // Kun 'amt1' er tilgjengelig uten ID-parameter
            "Amtskart": L.tileLayer.wms('https://wms.geonorge.no/skwms1/wms.historiskekart', {
                layers: 'amt1',  // Verifisert lagnavn fra GetCapabilities
                format: 'image/png',
                transparent: true,
                version: '1.1.1',  // KRITISK: Må være 1.1.1
                opacity: 0.5,
                crs: L.CRS.EPSG25833,  // Norsk koordinatsystem
                minZoom: 5,
                maxZoom: 18,
                attribution: '© Kartverket',
                className: 'historical-map-layer'
            }),

            // --- FØLGENDE LAG KREVER GEOID-TILGANG (NÅ AKTIVERT) ---
    ...
    function getOrCreateOpacitySlider() {
        ...
        let sliderContainer = menuContainer.querySelector('.integrated-opacity-control');

        if (!sliderContainer) {
            sliderContainer = L.DomUtil.create('div', 'integrated-opacity-control', menuContainer);
            ...
            sliderContainer.innerHTML = `
                <style>
                    ...
                    .integrated-opacity-value-display span {
                        font-size: 14px;
                        color: #38bdf8;
                    }
                </style>
                <div>
                    ...
                    <div class="integrated-opacity-value-display">
                        <span id="opacity-value">50</span><span style="color: #38bdf8;">%</span>
                    </div>
                </div>
            `;
            ...
            if (slider && valueDisplay) {
                slider.addEventListener('input', function () {
                    const transparency = this.value / 100;
                    const opacity = 1 - transparency;
                    valueDisplay.textContent = this.value;
                    if (activeHistoricalLayer) {
                        activeHistoricalLayer.setOpacity(opacity);
                    }
                });
            }
        }
    ...
    function updateOpacitySliderVisibility() {
        const container = getOrCreateOpacitySlider();
        if (!container) return;
        ...
        if (layerControl) {
            ...
            if (activeHistoricalLayer && isMenuExpanded) {
    ...
    map.on('overlayadd', function (e) {
        // Endret sjekk: Slider skal nå KUN aktiveres for Amtskartet
        if (e.name === "Amtskart") {
            activeHistoricalLayer = e.layer;
    ...
    if (flyfotoList) {
        Object.keys(flyfotoLayers).forEach((name, index) => {
            ...
            radio.addEventListener('change', () => {
                if (radio.checked) {
                    // Remove other flyfoto/base layers
                    map.eachLayer(layer => {
                ...
                if (isComplex) {
                    ...
                    const iframeHtml = `
                        <div style="font-family:'Outfit',sans-serif; width:100%; min-width:300px;">
                            ...
                            <iframe src="${proxiedUrl}" style="width:100%; height:450px; border:none; border-radius:10px; background:white;"></iframe>
                        </div>`;
...
function openModal(url, title, siteId = null, imgIndex = 0, htmlContent = null) {
    ...
    if (htmlContent) {
        ...
        if (isImage) {
            ...
            if (frame) {
                frame.style.display = 'block';
                // VIKTIG: Fjern srcdoc hvis den henger igjen fra forrige videovisning
            ...
            if (frame) {
                ...
                if (finalUrl.includes('kulturminnesok.no')) {
...
function updateVisibleMarkers() {
    ...
    allSites.forEach(site => {
        ...
        if (isVisible || isOpen) {
            ...
            if (site.lineHandle && !markerLayer.hasLayer(site.lineHandle)) {
                markerLayer.addLayer(site.lineHandle);
            }
        } else {
            if (markerLayer.hasLayer(site.marker)) {
                markerLayer.removeLayer(site.marker);
...
function summarizeStats() {
    const stats = {}; allSites.forEach(s => { stats[s.catKey] = (stats[s.catKey] || 0) + 1; });
...
function displayDailyHighlight() {
    const sitesWithImages = allSites.filter(s => s.images.length > 0); if (sitesWithImages.length === 0) return;
    const section = document.getElementById('daily-photo-section'); const container = document.getElementById('daily-photo-container');
    const site = sitesWithImages[Math.floor(Math.random() * sitesWithImages.length)];
    const safeName = getSafeName(site.name);
    const localImg = `bilder/${safeName}_1.jpg`;
    container.innerHTML = `<img src="${localImg}" alt="${site.name}" style="width:100%; border-radius:8px;" onerror="this.onerror=null; this.src='image_proxy.php?url=${encodeURIComponent(site.images[0])}';"><div style="margin-top:10px;"><strong>${site.category.icon} ${site.category.name}</strong><br><span style="font-size:0.9em; opacity:0.8;">${site.name}</span></div>`;
    container.onclick = () => focusSite(site.id); section.style.display = 'block';
}
...
function addMarker(site) {
    ...
    if (site.isLine) {
        ...
        const handle = L.circleMarker(site.coordsArray[0], { radius: 8, fillColor: site.category.color, color: '#fff', weight: 2, fillOpacity: 1 }).addTo(markerLayer);
    ...
    marker.bindTooltip(`${site.name}<br><span style="font-size: 0.8em; color: #aaa;">${site.lat.toFixed(5)}, ${site.lng.toFixed(5)}</span>`, { direction: 'top', offset: [0, -20], opacity: 0.9, className: 'custom-tooltip' });
    marker.bindTooltip(`${site.name}<br><span style="font-size: 0.8em; color: #aaa;">${site.lat.toFixed(5)}, ${site.lng.toFixed(5)}</span>`, { direction: 'top', offset: [0, -20], opacity: 0.9, className: 'custom-tooltip' });
    site.marker = marker;
...
function getPopupHTML(site) {
    ...
    let imageSection = displayUrl ? `
        <div class="image-carousel" style="cursor: pointer; position: relative; margin-bottom: 5px;" title="Klikk for å se bildet stort" onclick="openModal('${displayUrl}', '${site.name}', ${site.id}, ${curIdx})">
            ...
            <div class="zoom-hint" style="position: absolute; top: 10px; right: 10px; background: rgba(0,0,0,0.5); padding: 5px; border-radius: 50%; opacity: 0; transition: opacity 0.3s;">🔍</div>
        </div>` : '';
    ...
    let desc = site.description.replace(/\n/g, '<br>');
    // Remove "Nettside: ... askeladden..." text entirely
    desc = desc.replace(/Nettside:\s*https?:\/\/data\.kulturminne\.no\/askeladden\/lokalitet\/[^\s<]+(\s*og\s*)?/gi, '');

    desc = desc.replace(/(?<!href="|">)https?:\/\/[^\s<"']+/g, (m) => `<span class="inline-link" onclick="openModal('${m}', '${site.name}')" style="color: var(--accent-color); text-decoration: none; border-bottom: 2px solid var(--accent-glow); cursor: pointer; font-weight: 600; padding: 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; max-width: 100%; vertical-align: bottom;">${shortenLink(m)}</span>`);
    desc = desc.replace(/<a [^>]*href="([^"]+)"[^>]*>(.*?)<\/a>/g, (m, u, l) => `<span class="inline-link" onclick="openModal('${u}', '${site.name}')" style="color: var(--accent-color); text-decoration: none; border-bottom: 2px solid var(--accent-glow); cursor: pointer; font-weight: 600; padding: 0 2px; white-space: nowrap; overflow: hidden; text-overflow: ellipsis; display: inline-block; max-width: 100%; vertical-align: bottom;">${l}</span>`);

    site.links.forEach(link => {
        // Filter out obsolete Bygdeborg links
        if (link.includes('bora.uib.no')) return;

        // Transform Askeladden links to Kulturminnesøk
        if (link.includes('data.kulturminne.no/askeladden/lokalitet/')) {
            const match = link.match(/\/lokalitet\/(\d+)/);
            if (match && match[1]) {
                const lokalitetId = match[1];
                const nyLink = `https://www.kulturminnesok.no/kart/?id=${lokalitetId}`;
                desc += `<br><button onclick="openModal('${nyLink}', '${site.name}')" style="margin-top: 15px; width: 100%; padding: 14px; background: linear-gradient(135deg, rgba(56, 189, 248, 0.2), rgba(56, 189, 248, 0.1)); border: 1px solid rgba(56, 189, 248, 0.4); color: white; border-radius: 12px; cursor: pointer; font-weight: 700; font-family: 'Outfit', sans-serif; display: flex; align-items: center; justify-content: center; gap: 10px; transition: all 0.2s;">\ud83d\udd17 Les mer</button>`;
            }
            return; // Skip default button generation
        }

        const isVideo = link.includes('youtube.com') || link.includes('youtu.be') || link.includes('rz7ON7pVRTc');
        const isAlbum = link.includes('photos.app.goo.gl');
        const isImageHost = link.match(/\.(jpg|jpeg|png|gif|webp|bmp)(\?|#|$)/i) || link.includes('googleusercontent.com');

        // Vis "Se Album" selv om det er bilder (brukeren vil ofte se albumet i full kvalitet uansett)
        if (site.images.length > 0) {
            if (isImageHost && !isVideo && !isAlbum) return;
            if (site.images.some(img => img.includes(link) || link.includes(img)) && !isAlbum) return;
        }
        if (link.includes('SkPQzkgqQ3E6zR7n6') || link.includes('ZKktL6ESvvG6rdcN9')) return;
        let label = isAlbum ? 'Se Album' : isVideo ? '\ud83d\udcf9 Se Video' : '\ud83d\udcc4 Les Mer';
        desc += `<br><button onclick="openModal('${link}', '${site.name}')" style="margin-top: 15px; width: 100%; padding: 14px; background: linear-gradient(135deg, rgba(56, 189, 248, 0.2), rgba(56, 189, 248, 0.1)); border: 1px solid rgba(56, 189, 248, 0.4); color: white; border-radius: 12px; cursor: pointer; font-weight: 700; font-family: 'Outfit', sans-serif; display: flex; align-items: center; justify-content: center; gap: 10px; transition: all 0.2s;">\ud83d\udd17 ${label}</button>`;
    });

    // Special handling for Bygdeborg PDF
    if (site.catKey === 'BYGDEBORG') {
        desc += `<br><button onclick="openModal('bygdeborger.pdf', 'Bygdeborger i Telemark (PDF)')" style="margin-top: 15px; width: 100%; padding: 14px; background: linear-gradient(135deg, rgba(56, 189, 248, 0.2), rgba(56, 189, 248, 0.1)); border: 1px solid rgba(56, 189, 248, 0.4); color: white; border-radius: 12px; cursor: pointer; font-weight: 700; font-family: 'Outfit', sans-serif; display: flex; align-items: center; justify-content: center; gap: 10px; transition: all 0.2s;">📄 Les PDF</button>`;
    }

    return `<div class="popup-content" style="word-break: break-word;"><style>.inline-link:hover { filter: brightness(1.2); } .image-carousel img { width: 100%; border-radius: 12px; display: block; } .image-carousel:hover img { transform: scale(1.02); } .image-carousel:hover .zoom-hint { opacity: 1 !important; }</style>${html}${imageSection}<div style="margin-top: 18px; font-size: 1.1rem; line-height: 1.6; color: #f1f5f9;">${desc}</div></div>`;
}
...
window.toggleSearchHelp = function () {
    const helpText = `
        <div style="color:white; font-family:'Outfit', sans-serif; font-size:13px; line-height:1.5; padding:5px; max-width:300px;">
    ...
    L.popup().setLatLng(map.getCenter()).setContent(helpText).openOn(map);
};
